...
export async function initBirdsGame(socket) {
  ...
  const greenBirdEntity = new BirdEntity(
  ...
  const drawingPad = new DrawingPad(drawingPadCanvas, canvas, blueBirdEntity);
  ...
  function gameLoop(currentTime) {
    ...
    // Enviar la posición del pájaro azul al otro jugador solo si ha cambiado
    const currentBirdState = {
      x: blueBirdEntity.x,
      y: blueBirdEntity.y,
    ...
    if (hasPositionChanged(currentBirdState, lastBirdUpdate)) {
      socket.emit("blueBirdUpdate", currentBirdState);
      lastBirdUpdate = { ...currentBirdState };
    }
